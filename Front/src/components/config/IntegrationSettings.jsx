...
const fallbackIcon = <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><line x1="2" y1="12" x2="22" y2="12" /><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z" /></svg>;

export default function IntegrationSettings() {
  const [integs, setIntegs] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchIntegs = useCallback(async () => {
    try {
  ...
  useEffect(() => { fetchIntegs(); }, [fetchIntegs]);

  async function toggleIntegration(id) {
    try {
      const { data } = await api.patch(`/integrations/${id}/toggle`);
      setIntegs((prev) => prev.map((i) => i.id === id ? { ...i, status: data.status } : i));
    } catch (err) {
      console.error("Erro ao alternar integração:", err);
    }
  }

  const connected = integs.filter((i) => i.status === "conectado").length;

  if (loading) return <div className="py-12 text-center text-slate-400 text-sm">Carregando integrações...</div>;
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-[11px] text-slate-400 mt-0.5">
            Conecte ferramentas do ecossistema — {connected} de {integs.length} ativas
          </p>
        </div>
      </div>

      {integs.length === 0 ? (
        <div className="py-10 text-center text-slate-400 text-sm">Nenhuma integração cadastrada.</div>
      ) : (
        <div className="grid grid-cols-2 gap-3">
          {integs.map((integ) => {
            const isConnected = integ.status === "conectado";
            const icon = iconMap[integ.icon] || fallbackIcon;

            return (
              <div key={integ.id} className={`flex items-center gap-4 p-4 rounded-lg border transition-colors ${isConnected ? "border-green-200 bg-green-50/30" : "border-slate-200 bg-white"}`}>
